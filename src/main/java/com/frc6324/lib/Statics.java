...
public final class Statics {
    ...
    /**
     * The default exception handler used by {@link Statics#initOrDefault(ThrowableSupplier, Supplier)}. This reports the error to DriverStation with the error's filled-in stack trace.
     *
     * @param <E>   The type of exception being handled (this does not affect how the exception is handled).
     ...
     * @param defaultSupplier The default value to use if {@code init} throws something. This will run unchecked, so throwing an exception in this function will cause your robot program to crash if it is not properly handled.
     * @return The initialized value.
     */
    public static <T> T initOrDefault(ThrowableSupplier<T, Exception> init, Supplier<T> defaultSupplier) {
        // Use the default exception handler
        return initOrDefault(init, defaultSupplier, Statics::defaultExceptionHandler);
    }

    /**
     * Attempts to initialize a value with one constructor, using a given exception handler and default supplier if it throws an exception.
     *
     * @param <T>              The type of the value being initialized.
    ...
    public static <T, E extends Exception> T initOrDefault(ThrowableSupplier<T, E> init, Supplier<T> defaultSupplier, Consumer<E> exceptionHandler) {
        try {
            // Try to get 
            return init.get();
        } catch (Exception e) {
     ...
     * @param <E> The type of exception that can be thrown. Setting this to {@link Exception} will allow any type of exception to be thrown.
     */
    @FunctionalInterface
