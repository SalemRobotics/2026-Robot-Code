...
/**
 * An extension of {@link BeamBreakIODIO} that adds a glitch filter that can reduce noise directly
 * in the RoboRIO's FPGA.
 *
 * <p>Because of limitations in the RoboRIO's FPGA, only 3 glitch filters can be created for a whole
 * robot. If this limit is exceeded, an exception is thrown.
 */
public class BeamBreakIOFilteredDIO extends BeamBreakIODIO {
  /** The FPGA glitch filter being used. */
  @Getter private final DigitalGlitchFilter filter;

  /**
   * Creates a new beam break sensor using a real digital I/O (DIO) device.
   *
   * <p>The caller of this function is responsible for setting the amount of time the sensor must
   * report a value for before the FPGA propogates the new value.
   *
   ...
   */
  public BeamBreakIOFilteredDIO(
      DigitalInput dioDevice, DigitalGlitchFilter filter, boolean inverted) {
   ...
   *
   * @param dioDevice The Digital I/O device to read.
   * @param steadyTime The minimum time a sensor needs to maintain a state before the FPGA
   *     propogates it to the robot code.
   * @param inverted Whether the sensor is inverted.
   */
