...
public class ClimberIOTalonFX implements ClimberIO {
    ...
    private final StatusSignalCollection signals = new StatusSignalCollection(motorVelocity, motorVoltage, motorStatorCurrent, motorSupplyCurrent, motorTorqueCurrent);

    /**
     * Creates an I/O implementation for the climber backed by a TalonFX motor controller.
     */
    public ClimberIOTalonFX() {
        PhoenixUtil.tryUntilOk(5, () -> talon.getConfigurator().apply(CLIMBER_CURRENT_LIMITS, 0.25), "Failed to configure the climber motor's current limits");
        PhoenixUtil.tryUntilOk(5, () -> talon.setNeutralMode(NeutralModeValue.Brake), "Failed to configure the climber motor's neutral mode.");
    }
