...
public final class DrivingUtils {
    ...
    /**
     * The amount of consideration to use when guessing the robot's future tilt using lateral acceleration
     */
    private static final double ACCELERATION_WEIGHT = 0.3;

    /**
     * The amount of consideration to use when guessing the robot's future tilt using angular velocities
     */
     ...
     */
    private static final Notification TILT_NOTIFICATION = new Notification()
     ...
     *
     * @param roll          The roll (rotation about the X axis) experienced by the
     *                      robot.
     * @param pitch         The pitch (rotation about the Y axis) experienced by the
     *                      robot.
    ...
    /**
     * Checks if the robot is going to tip over in the forseeable future. This is
     * not 100% accurate
     * (as it cannot take into account the robot colliding with another object) and
     * shouldn't be used
     * to majorly affect the robot.
     *
     * @return Whether or not the robot should limit its speed because it is going
     *         to tip over.
     */
    ...
    private static double predictRoll() {
        ...
        double accelerationRoll = rollDegrees
                + Math.toDegrees(Math.atan2(accelYMPSsq, GRAVITY_CONSTANT)) * LOOKAHEAD_TIME;
    ...
    private static double predictPitch() {
        ...
        double accelerationPitch = pitchDegrees
                + Math.toDegrees(Math.atan2(accelXMPSsq, GRAVITY_CONSTANT)) * LOOKAHEAD_TIME;
    ...
    public static void updateOdometry(Pose2d currentPose, ChassisSpeeds currentSpeedsRobotRelative) {
        ...
        DrivingUtils.currentSpeeds = ChassisSpeeds.fromRobotRelativeSpeeds(currentSpeedsRobotRelative, currentPose.getRotation());
    }
     ...
     */
    public Pose2d estimateFuturePose(double lookaheadPeriod) {
        Translation2d translationRate = new Translation2d(
            currentSpeeds.vxMetersPerSecond + accelXMPSsq * lookaheadPeriod,
            currentSpeeds.vyMetersPerSecond + accelYMPSsq * lookaheadPeriod);
        Translation2d translation = currentPose.getTranslation().plus(translationRate.times(lookaheadPeriod));
