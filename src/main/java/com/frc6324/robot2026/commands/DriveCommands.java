...
public final class DriveCommands {
    ...
    /**
     * Calculates the linear velocity the drivetrain should follow given joystick
     * values.
     * 
     * @param x The X input.
     * @param y The Y input.
    ...
    public static Command driveToPose(SwerveDrive drive, Pose2d pose) {
        ...
        return drive.defer(() -> {
            var target = new Path.Waypoint(pose, true);
    ...
    public static Command drivePointingTowards(SwerveDrive drive, XboxController controller, Pose2d target) {
        ...
        return drive.run(() -> {
            ...
            Translation2d linearVelocity = getLinearVelocityFromJoysticks(-controller.getLeftY(),
                    -controller.getLeftX());
            ...
            var diff = target.getTranslation().minus(drive.getPose().getTranslation());
            // Calculate the angle that delta needs
            var facing = new Rotation2d(diff.getX(), diff.getY());
    ...
    public static Command joystickDrive(SwerveDrive drive, XboxController controller) {
        ...
        return drive.run(() -> {
            // Get the linear velocity
            Translation2d linearVelocity = getLinearVelocityFromJoysticks(-controller.getLeftY(), -controller.getLeftX());
            // Multiply the linear velocity by the drivetrain's max speed
            linearVelocity = linearVelocity.times(SwerveDrive.getMaxLinearSpeed());
