...
public final class DriveCommands {
  ...
  static final double TRANSLATION_MAX_VELOCITY = SwerveDrive.getMaxLinearSpeedMetersPerSec();
  static final double TRANSLATION_MAX_ACCELERATION = 7;
  static final double ANGLE_MAX_VELOCITY = SwerveDrive.getMaxAngularSpeedRadPerSec();
  ...
  static final TrapezoidProfile.Constraints POINTING_CONSTRAINTS =
   ...
   */
  public static Translation2d getLinearVelocityFromJoysticks(final double x, final double y) {
  ...
  public static Translation2d getLinearVelocityFromJoysticks(final XboxController controller) {
  ...
  public static Command joystickDrive(final SwerveDrive drive, final XboxController controller) {
    return drive.run(
        () -> {
          ...
          linearVelocity = linearVelocity.times(SwerveDrive.getMaxLinearSpeedMetersPerSec());
          ...
          omega *= SwerveDrive.getMaxAngularSpeedRadPerSec();
          ...
          drive.runVelocity(
              ChassisSpeeds.fromFieldRelativeSpeeds(
                  linearVelocity.getX(), linearVelocity.getY(), omega, drive.getRotation()));
        });
  }
}
