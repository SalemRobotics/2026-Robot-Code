...
public final class AutoOperateCommand extends Command {
  ...
  private final SwerveRequest.FieldCentric regularDrive =
  ...
  private void driverControl() {
    ...
    drive.setControl(
        regularDrive.withVelocityX(translation.getX()).withVelocityY(translation.getY()));
  }
  ...
  private void executeScoringMode() {
    if (rawHID.getRightTriggerAxis() > TRIGGER_THRESHOLD) {
      ...
      final Translation2d robotToHub =
          Hub.TOP_CENTER_POINT.toTranslation2d().minus(robotPose.getTranslation());
      final Rotation2d facing = robotToHub.getAngle();

      final Translation2d translation =
          DriveCommands.getLinearVelocityFromJoysticks(rawHID)
      ...
      if (MathUtil.isNear(
          facing.getRadians(), robotPose.getRotation().getRadians(), HUB_ROTATION_TOLERANCE)) {
